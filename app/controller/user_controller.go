...
import (
	...
	"github.com/irhendra09/chat-app/app/repository"
	"github.com/irhendra09/chat-app/pkg/jwt_token"
	"github.com/irhendra09/chat-app/pkg/response"
...
func Register(ctx fiber.Ctx) error {
	...
	err = repository.InsertNewUser(user)
	if err != nil {
...
func Login(ctx fiber.Ctx) error {
	...
	user, err := repository.GetUserByUsername(loginReq.Username)
	if err != nil {
	...
	token, err := jwt_token.GenerateToken(user.Username, user.FullName, "token", now)
	if err != nil {
	...
	refreshToken, err := jwt_token.GenerateToken(user.Username, user.FullName, "refresh_token", now)
	if err != nil {
		errResponse := fmt.Errorf("failed to generate token: %v", err)
		log.Println(errResponse)
		return response.SendFailureResponse(ctx, fiber.StatusInternalServerError, "terjadi kesalahan pada sistem", nil)
	...
	err = repository.InsertNewUserSession(userSession)
	if err != nil {
...
func Logout(ctx fiber.Ctx) error {
	...
	err := repository.DeleteUserSessionByToken(token)
	if err != nil {
...
func RefreshToken(ctx fiber.Ctx) error {
	...
	token, err := jwt_token.GenerateToken(username, fullName, "token", now)
	if err != nil {
		errResponse := fmt.Errorf("failed to generate token: %v", err)
		log.Println(errResponse)
		return response.SendFailureResponse(ctx, fiber.StatusInternalServerError, "terjadi kesalahan pada sistem", nil)
	...
	err = repository.UpdateUserSessionToken(token, now.Add(jwt_token.MapTypeToken["token"]), refreshToken)
	if err != nil {
